...
public class ProxyUtils {
	...
	private static void init() {
		...
		try {
			...
			while (localAddrs.hasMoreElements()) {
				...
				if (!tmp.isLoopbackAddress() && !tmp.isLinkLocalAddress()
	...
	public static boolean validateProxy(HttpHost p) {
		...
		try {
			...
			InetSocketAddress endpointSocketAddr = new InetSocketAddress(p
					.getAddress().getHostAddress(), p.getPort());
			socket.connect(endpointSocketAddr, 3000);
			logger.debug("SUCCESS - connection established! Local: "
					+ localAddr.getHostAddress() + " remote: " + p);
			isReachable = true;
		} catch (IOException e) {
			logger.warn("FAILRE - CAN not connect! Local: "
					+ localAddr.getHostAddress() + " remote: " + p);
		} finally {
			if (socket != null) {
				try {
					...
					logger.warn(
							"Error occurred while closing socket of validating proxy",
							e);
				}
			}
		}
		...
		while (enumeration.hasMoreElements()) {
			...
			while (addr.hasMoreElements()) {
				...
				if (s != null && IPV4_PATTERN.matcher(s).matches()) {
					networkInterfaceName += networkInterface.toString() + "IP:"
							+ s + "\n\n";
				}
			}
		}
