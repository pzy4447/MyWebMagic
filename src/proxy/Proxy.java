...
public class Proxy implements Delayed, Serializable {
	...
	Proxy(HttpHost httpHost, String user, String password) {
		...
		this.canReuseTime = System.nanoTime()
				+ TimeUnit.NANOSECONDS.convert(reuseTimeInterval,
	...
	Proxy(HttpHost httpHost, int reuseInterval, String user, String password) {
		...
		this.password = password;
		this.canReuseTime = System.nanoTime()
				+ TimeUnit.NANOSECONDS.convert(reuseInterval,
						TimeUnit.MILLISECONDS);
	}
	...
	public long getDelay(TimeUnit unit) {
		return unit.convert(canReuseTime - System.nanoTime(),
	...
	public int compareTo(Delayed o) {
		Proxy that = (Proxy) o;
		return canReuseTime > that.canReuseTime ? 1
				: (canReuseTime < that.canReuseTime ? -1 : 0);
	...
	public String toString() {
		...
		String re = String.format(
				...
				httpHost.getAddress().getHostAddress(), responseTime,
